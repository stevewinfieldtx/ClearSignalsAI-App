  ...
  const analyze = async () => {
    ...
    try {
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 4000, system: SYS_PASTE,
...
function DatabaseMode({ data, contacts, savedResults, setSavedResults, onBack }) {
  ...
  const analyzeNext = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 2000, system: SYS_FULL, messages: msgs })
      });
      const d = await resp.json();
      const raw = d.content?.map(b=>b.text||"").join("")||"";
  ...
  const analyzeDeciding = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 3000, system: SYS_DECIDING,
